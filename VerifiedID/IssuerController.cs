...
namespace AspNetCoreVerifiableCredentials
{
    ...
    public class IssuerController : ControllerBase
    {
        ...
        /// <summary>
        /// This method is called from the UI to submit a new image to the biometric service
        /// </summary>
        /// <returns>200 OK or error</returns>

        [HttpPost("/api/issuer/submit-image")]
        public async Task<ActionResult> SubmitImage([FromForm] IFormFile image, [FromForm] string session)
        {
            try
            {
                ...
                // if userDetails did not exist, then it is not called by the Interstellar AAD / front-end website. Return error.
                if (session == null) {
                    return BadRequest(new { error = "400", error_description = "Session is missing, this version of the Issuer must be called with user details from AAD" });
                }

                // convert User Details to object
                byte[] sessionDecoded = Convert.FromBase64String(session);  // decode from Base64 to JSON stringified object
                string sessionJsonString = Encoding.UTF8.GetString(sessionDecoded); // convert from byte array to string
//                var userDetails = JsonConvert.DeserializeObject(sessionJsonString); // convert from JSON stringified object to in-memory object

                // put in object and store in cache
                var cacheData = new
                {
                    userDetails = sessionJsonString,
                    image = imageBase64
                };
        ...
        public async Task<ActionResult> IssuanceRequest()
        {
            try
            {
                ...
                // STOP HERE. Next steps:
                //  We got the state for the cache, the cache contains the image.
                //  We now need to load this image from the cache and into the credential, then we may need to clear the cache before it is used by other parts of the code

                // load image from cache and convert from Base64 to byte array
                byte[] imageByteContent = null;
                JObject value = null;
                string userDetailsString = null;
                if (_cache.TryGetValue(state, out string buf)) {
                    value = JObject.Parse(buf);
                    userDetailsString = (string)value["userDetails"];                    
                ...
                var userDetails = JObject.Parse(userDetailsString);
...
//                string state = Guid.NewGuid().ToString();
                ...
                // set the VC payload
                payload["issuance"]["claims"]["name"] = userDetails["name"];
                payload["issuance"]["claims"]["username"] = userDetails["username"];
                payload["issuance"]["claims"]["oid"] = userDetails["oid"];
                payload["issuance"]["claims"]["tenantId"] = userDetails["tenantId"];
                payload["issuance"]["claims"]["faceId"] = faceId;
